...
class SoftmaxOnline:
    ...
    def __call__(self, gInput: cute.Tensor, gOutput: cute.Tensor, stream: cuda.CUstream = None):
        ...
        tiler_mn = (self.num_warps, self.vec_load_size * blocks_over_N * self.threads_per_row) # [4, ~N]
    ...
    def kernel(self, gInput: cute.Tensor, gOutput: cute.Tensor, tiler_mn: cute.Shape, tiled_copy: cute.TiledCopy):
        ...
        tidLocalMax = tidxValues.reduce(cute.ReductionOp.MAX, init_val=self.NEG_INF, reduction_profile=0)
        rowMax = cute.arch.warp_reduction_max(tidLocalMax)
        tidScaledLocalSum = cute.math.exp(tidxValues - rowMax)
        tidLocalSum = tidScaledLocalSum.reduce(cute.ReductionOp.ADD, init_val=0.0, reduction_profile=0)
